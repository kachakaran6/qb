...
function App() {
  ...
  const [view, setView] = useState<"subjects" | "questions" | "answer">(
    "subjects"
  );
  ...
  const activeQuestionData =
    activeSubjectData && activeQuestion
      ? activeSubjectData.questions.find(
          (question) => question.id === activeQuestion
        )
      : null;

  const activeQuestionIndex =
    activeSubjectData?.questions.findIndex((q) => q.id === activeQuestion) ??
    -1;
  const hasNextQuestion =
    activeSubjectData &&
    activeQuestionIndex < activeSubjectData.questions.length - 1;
  ...
  const handleNextQuestion = () => {
    if (
      ...
      setActiveQuestion(
        activeSubjectData.questions[activeQuestionIndex + 1].id
      );
    }
  };
